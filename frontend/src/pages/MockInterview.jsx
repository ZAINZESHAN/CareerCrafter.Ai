...
const MockInterview = () => {
  ...
  const generateQuestions = async () => {
    setLoading(true);
    setScore(null);
    setIsSubmitted(false); // Reset submission status when generating questions
    try {
  ...
  const reset = () => {
    ...
    setJobRole("");
    setIsSubmitted(false); // Reset submission status on reset
  };
        ...
        score !== null && isSubmitted && (
          <div className="flex justify-center mt-4 text-lg font-semibold gap-2 text-green-600">
            ...
            <Button
              danger
              onClick={reset}
              size="large" // Increase the button size using Ant Design's size prop
